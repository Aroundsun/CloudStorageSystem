...
namespace storage
{
    ...
    public:
        ...
        bool ReadConfig()
        {
            logsystem::GetLogger("asynclogger")->Info("ReadConfig start");

            storage::FileUtil fu(Config_File);
            std::string content;
            if (!fu.GetFileContent(&content))
            {
                logsystem::GetLogger("asynclogger")->Error("GetFileContent Failed: %s", Config_File);
                return false;
            }
